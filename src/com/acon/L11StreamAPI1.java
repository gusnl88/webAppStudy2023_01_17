...
public class L11StreamAPI1 {
    public static void main(String[] args) {
        ...
        // 때문에 반복문을 재사용하는 것이 어려워졌다.(반복문의 특정 코드를 함수로 변형해서 재사용할 수 있지만 반복문 자체는 재사용하기 힘들다.)
        ...
        // 이터레이터와 유사한 자료로 변환할 수 있도록 정의하고 Stream 의 필드로 내부함수를 정의했다.
        // 내부함수를 크게 3가지로 나누는데 중간연산, 최종연산, 컬렉트연산으로 나누어 서로 다른 역할을 부여했다.
        // 특히 중간연산은 연산의 결과가 Stream 으로 중간연산을 연결함으로서 반복문을 중첩하는 효과를 갖는다.

        /*Stream AIP 중간 연산 (중간연산은 메소드체이닝을위해 무조건 Stream 을 반환하고 최종연산과 같이 사용해야한다.)
        * distinct : 중복을 제거 (기본형+자료형(equals 를 구현해야함))
        * map(Function) : 스트림의 각요소를 변형하기 위한 반복문 (매개변수가 현재 요소고 반환하는 것이 바뀌는 요소)
        * */
